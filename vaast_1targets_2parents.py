...
#Options
parser = OptionParser()
parser.add_option('-a', help='vcf', dest='affected') #, nargs=2
...
command = "perl %s/vaast_tools/vaast_converter -f VCF --path gvf/ -b hg19 %s" % (vaast_dir, affected)
os.system(command)
...
##VAT
print "Start VAT"
command = "%s/VAT -f %s -a %s -b hg19 gvf/Exome_1_RP.gvf > gvf/child.vat.gvf" % (vaast_dir, genes, reference)
os.system(command)
command = "%s/VAT -f %s -a %s -b hg19 gvf/father.gvf > gvf/father.vat.gvf" % (vaast_dir, genes, reference)
os.system(command)
command = "%s/VAT -f %s -a %s -b hg19 gvf/mother.gvf > gvf/mother.vat.gvf" % (vaast_dir, genes, reference)
os.system(command)
...
##VST Affecteds
command = "%s/VST -o 'U(0)' -b hg19 gvf/child.vat.gvf > child.cdr" % (vaast_dir)
os.system(command)


##VST Parents
command = "%s/VST -o 'U(0..1)' -b hg19 gvf/father.vat.gvf gvf/mother.vat.gvf > parents.cdr" % (vaast_dir)
os.system(command)
die()
