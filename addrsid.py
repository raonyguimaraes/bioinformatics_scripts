...
parser.add_option("-i", dest="input_file",
                  help="vcffile", metavar="VCF") #, nargs=12
...
dbsnp="/lgc/datasets/dbsnp/142/00-All.vcf"

gatk_dir="/lgc/programs/GenomeAnalysisTK-3.1-1"

# command = "java -Xmx10G -jar %s/GenomeAnalysisTK.jar \
# -T VariantAnnotator \
# -R %s \
# --dbsnp %s \
# --alwaysAppendDbsnpId \
...
# -L %s \
# -o %s \
# " % (gatk_dir, reference, dbsnp, options.input_file, options.input_file, options.output)
command = "java -jar /lgc/mendelmd_master/annotator/libs/snpeff/snpEff/SnpSift.jar annotate -id %s %s > %s" % (dbsnp, options.input_file, options.output)
print command
