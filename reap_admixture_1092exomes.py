...
#compress
command = '/lgc/programs/tabix-0.2.6/bgzip %s' % (filtered_file)
os.system(command)

#index
command = 'tabix -p vcf %s.gz' % (filtered_file)
os.system(command)
...
command = '''/lgc/programs/vcftools_0.1.10/bin/vcftools \
         ...
         --phased \
         --remove-indels \
         --recode --out  %s.ceu_afr_amr.no_missing''' % (filtered_file, filename)
# print command
os.system(command)
