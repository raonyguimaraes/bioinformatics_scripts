...
#running program
#python /lgc/scripts/vaast_2targets_2parents.py -a /projects/exomes030113/Exome_EAM.var.annotated.vcf /projects/exomes030113/Exome_CER.var.annotated.vcf --ag male male -p /projects/exomes030113/Exome_ECR.var.annotated.vcf /projects/exomes030113/Exome_ATM.var.annotated.vcf --pg male female
...
parser.add_option('--ag', help='male/female', dest='agender', nargs=2) #, nargs=2
parser.add_option('-p', help='vcf', dest='parents', nargs=2) #, nargs=2
parser.add_option('--pg', help='male/female', dest='pgender', nargs=2) #, nargs=2


(options, args) = parser.parse_args()

affecteds=options.affecteds
affecteds_gender=options.agender

parents=options.parents
parents_gender=options.pgender

affecteds_names = []
for item in affecteds:
  affecteds_names.append(item.split('/')[-1].split('.')[0])

parents_names = []
for item in parents:
  parents_names.append(item.split('/')[-1].split('.')[0])


#VAAST ANALYSIS 1.0.1

vaast_dir="/lgc/programs/VAAST_Code_1.0.4/bin"
...
# command = "perl /lgc/programs/VAAST_Code_1.0.4/bin/vaast_tools/vaast_converter -f VCF --path gvf/ -b hg19 %s" % (affecteds[0])
# os.system(command)
# command = "perl /lgc/programs/VAAST_Code_1.0.4/bin/vaast_tools/vaast_converter -f VCF --path gvf/ -b hg19 %s" % (affecteds[1])
# os.system(command)
# command = "perl /lgc/programs/VAAST_Code_1.0.4/bin/vaast_tools/vaast_converter -f VCF --path gvf/ -b hg19 %s" % (parents[0])
# os.system(command)
# command = "perl /lgc/programs/VAAST_Code_1.0.4/bin/vaast_tools/vaast_converter -f VCF --path gvf/ -b hg19 %s" % (parents[1])
# os.system(command)
...
path = os.getcwd()+"/gvf"
#children1
# command = "%s/VAT --gender %s -f %s -a %s -b hg19 %s/%s.gvf > %s.vat.gvf" % (vaast_dir, affecteds_gender[0], genes, reference, path, affecteds_names[0], affecteds_names[0])
# os.system(command)

# command = "%s/VAT --gender %s -f %s -a %s -b hg19 %s/%s.gvf > %s.vat.gvf" % (vaast_dir, affecteds_gender[1], genes, reference, path, affecteds_names[1], affecteds_names[1])
# os.system(command)

# command = "%s/VAT --gender %s -f %s -a %s -b hg19 %s/%s.gvf > %s.vat.gvf" % (vaast_dir, parents_gender[0], genes, reference, path, parents_names[0], parents_names[0])
# os.system(command)


# command = "%s/VAT --gender %s -f %s -a %s -b hg19 %s/%s.gvf > %s.vat.gvf" % (vaast_dir, parents_gender[1], genes, reference, path, parents_names[1], parents_names[1])
# os.system(command)
...
print "Start VST"
##VST Affecteds
# command = "%s/VST -o 'U(0..1)' -b hg19 %s.vat.gvf %s.vat.gvf > targets.cdr" % (vaast_dir, affecteds_names[0], affecteds_names[1])

# os.system(command)


# ##VST Parents
# command = "%s/VST -o 'U(0..1)' -b hg19 %s.vat.gvf %s.vat.gvf > parents.cdr" % (vaast_dir, parents_names[0], parents_names[1])
# os.system(command)
...
simulations = []
#--indel to consider indels

#most Simple
command = "time %s/VAAST --indel -mp1 8 -lh n -pnt y -m lrt --codon_bias --gp 10000 --inheritance r --trio parents.cdr --outfile simulations/simulation_simple %s %s targets.cdr" % (vaast_dir, genes, background)
os.system(command)
