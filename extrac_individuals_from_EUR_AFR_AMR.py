...
for line in pedfile:
    ...
    if individual[1] in individuals:
        if individual[6] in AMR:
            populations['AMR'] = individual[1]
            indiv_list.append(individual[1])
        if individual[6] in AFR:
            populations['AFR'] = individual[1]
            indiv_list.append(individual[1])
        if individual[6] in EUR:
            populations['EUR'] = individual[1]
            indiv_list.append(individual[1])


    #if individual[6] not in populations:
	#populations[individual[6]] = []
    #populations[individual[6]].append(individual[1])
...
# for i in range(1,23):
    # print i
    # command = 'tabix -p vcf ALL.chr%s.integrated_phase1_v3.20101123.snps_indels_svs.genotypes.vcf.gz' % (i)
    # print command
    # os.system(command)
    # command = 'tabix ALL.chr%s.integrated_phase1_v3.20101123.snps_indels_svs.genotypes.vcf.gz -B %s -h > 1092exomes/chr%s.sureselectv4.vcf &' % (i,bedfile,i)
    # print command
    
    #this command sucks!!!
    # command = '/lgc/programs/vcftools_0.1.10/bin/vcftools --remove-indels --gzvcf ALL.chr%s.integrated_phase1_v3.20101123.snps_indels_svs.genotypes.vcf.gz --bed /lgc/datasets/exome_targets/SureSelectV4.withoutchr.bed --out 1092exomes/chr%s.sureselectv4.vcf --recode' % (i,i)
    # os.system(command)
...
#     print i
#     #compress
#     command = 'bgzip chr%s.sureselectv4.vcf' % (i)
#     os.system(command)
#     #index all vcfs with tabix
#     command = 'tabix -p vcf chr%s.sureselectv4.vcf.gz &' % (i)
#     os.system(command)
    exome_file_list.append('chr%s.sureselectv4.vcf.gz' % (i))

#VCFConcat
# command = '%s/vcf-concat %s | bgzip -c > 1092exomes.sureselectv4.vcf.gz' % (vcf_dir, " ".join(exome_file_list))
# os.system(command)
# command = 'tabix -p vcf 1092exomes.sureselectv4.vcf.gz'
# os.system(command)
# #vcfsubset
command = '%s/vcf-subset -c %s 1092exomes.sureselectv4.vcf.gz | bgzip -c > 1092exomes.EUR_AFR_AMR.sureselectv4.vcf.gz' % (vcf_dir, ",".join(indiv_list))
os.system(command)
...
# #continents = {'AMR' : AMR, 'AFR':AFR, 'EAS':EAS, 'EUR':EUR}
...
#     print individual
#     population = populations[individual]
#     continent = continents[populations[individual]]
    
#     command = "cat %s | %s/vcf-subset -c %s | bgzip -c > individuals/%s.%s.%s.vcf.gz" % (genomes1k_file, vcf_tools, individual, continent, population, individual)
#     #os.system(command)
