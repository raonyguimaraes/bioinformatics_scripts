...
parser.add_option("-v", dest="vcffile",
                  help="Output VCF", metavar="VCFFILE")
...
command = """
      java -Xmx40g -Djava.io.tmpdir=/projects/tmp -jar %s/GenomeAnalysisTK.jar -T ReadBackedPhasing \
      -l INFO \
      -I %s \
      -R %s \
      --variant %s \
      -o  %s\
      -log %s-ReadBackbyPhasing.log \
      -L %s \
      """ % (gatk_dir, " -I ".join(input_file), reference, options.vcffile, options.output, options.output, re.escape(target_array))
