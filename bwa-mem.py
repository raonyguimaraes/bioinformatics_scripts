...
reference="/home/raony/analysis/reference/human_g1k_v37.fasta"
...
class Bwa():
    ...
    def alignment(self):
      ...
      #%s/ bwa_dir
      command = "bwa mem -t 8 %s %s %s > exome.sam" % (reference, reads1, reads2)
    ...
    def sam_to_bam(self):
      ...
      #import BAM
      command = "samtools view -bS exome.sam > exome.bam"#, reference
      #samtools view -bS sample.sam > sample.bam
      #samtools import %s exome.sam exome.bam
      os.system(command)
      # #Sort BAM
      command = "samtools sort exome.bam exome.sorted"# % (st_dir)
      os.system(command)
      # #Index BAM
      command = "samtools index exome.sorted.bam exome.sorted.bam.bai"# % (st_dir)
      os.system(command)
      #Calmd
    ...
    def clean_files(self):
      command = "rm exome.sam exome.bam"# % (st_dir)
      os.system(command)
